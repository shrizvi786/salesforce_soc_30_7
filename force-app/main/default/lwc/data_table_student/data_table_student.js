...
// Import the CSS file
import styles from './data_table_student.css';
import getContactRecords from '@salesforce/apex/ActivityLogsSTUDENT.getContactRecords';


export default class Data_table_student extends LightningElement {
...
get disableNextButton() {
    return this.isLastPage || this.loading;
}

// Toggle functionality 
// get containerClass() {
//     return this.isFullScreen ? 'container full-screen' : 'container';
// }
get fullScreenClass() {
        ...
        .then((result) => {
            if (result) {
                ...
                if (this.data.length > 0) {
                   ...
                   // Remove "Teacher_Id" and "Email" columns from this.columnsStarting pgAdmin 4...
                   this.columns = this.columns.filter(column => column.fieldName !== 'Teacher_Id' && column.fieldName !== 'Email'  );
                     // Reverse the columns array
                     this.columns.reverse();
                     // Set the value of the new Duration (min./Sec) column
                    //  this.data.forEach((row, index) => {
                    //     if (row.Duration_minutes__c) {
                    //         const duration = Math.round(row.Duration_minutes__c * 10) / 10;
                    //         this.data[index].Duration_minutes__c = duration ;
                    //     }
                    // });
                    this.data.forEach((row, index) => {
                        if (row["Duration (Minutes)"]) {
                          const duration = Math.round(row["Duration (Minutes)"] * 100) / 100;
                          this.data[index]["Duration (Minutes)"] = duration;
                        }
                      });

                    //-------Change time format in eastern time zone----------
                        this.data.forEach((row, index) => {
                            if (row["Session Start"] && row["Session End"]) {
                            // Convert Session Start to Eastern time
                            const sessionStart = new Date(row["Session Start"]);
                            const sessionStartEastern = sessionStart.toLocaleString('en-US', {
                            ...
                            // Convert Session End to Eastern time
                            const sessionEnd = new Date(row["Session End"]);
                            const sessionEndEastern = sessionEnd.toLocaleString('en-US', {
                                timeZone: 'America/New_York',
                                hour12: false,     //time display in 24 hr format
                                hour: '2-digit',
                                minute: '2-digit',
                                second: '2-digit'
                            });
                        
                            // Update the data with formatted Eastern time values
                            this.data[index]["Session Start"] = sessionStartEastern;
                            this.data[index]["Session End"] = sessionEndEastern;
                            }
                        });


                      // Format the Date column to display only "MM-DD-YYYY"
