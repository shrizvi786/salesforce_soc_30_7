...
import getContactRecords from '@salesforce/apex/NewsInteractionRecords.showStudent';

export default class NewsInteraction extends LightningElement {
                                ...
                                if (result) {
                                        ...
                                        if (this.data.length > 0) {
                                                ...
                                                this.columns = Array.from(allKeys).map((column) => ({
                                                        ...
                                                        typeAttributes: column === 'Image' ? { label: 'View', target: '_blank' } : undefined,
                                                }));
        ...
        fetchData() {
                ...
                getContactRecords({ recordId: this.recordId, offset: offset, limits: this.limits })
                        .then((result) => {
                                ...
                                if (result.length < 5 && result.length === 0 && this.isFullScreen === false) {
                                        console.log('No more data:::::');
                                        this.disableNextButton = true;
                                        this.disableViewAll = true;
                                } else if (result.length < 100 && result.length === 0 && this.isFullScreen === true) {
