trigger SyncAssessmentAnalysisRollupAssessments on Assessment_Analysis_Rollup_Assessments__c(after insert, after update, before delete) {
    ...
    TriggerSFSyncAPI job = new TriggerSFSyncAPI(
        'Assessment_Analysis_Rollup_Assessments__c',
        new List<String>(),
        '',
        new List<SObject>()
    );
    if (Trigger.isInsert) {
        for (Assessment_Analysis_Rollup_Assessments__c record : Trigger.new) {
        ...
        for (Assessment_Analysis_Rollup_Assessments__c record : Trigger.old) {
