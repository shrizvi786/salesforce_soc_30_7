trigger ScheduleSchoolTypeBlackoutDatesRecord on Schedule_School_Type_Blackout_Dates__c (after insert, after update, before delete) {
    TriggerSFSyncAPI job = new TriggerSFSyncAPI('Schedule_School_Type_Blackout_Dates__c', new List<String>(), '',new List<SObject>());
    if (trigger.isInsert) {
        for (Schedule_School_Type_Blackout_Dates__c sch : Trigger.new) {
        ...
        for (Schedule_School_Type_Blackout_Dates__c sch : Trigger.old) {
