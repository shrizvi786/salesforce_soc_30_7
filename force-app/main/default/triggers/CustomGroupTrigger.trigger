trigger CustomGroupTrigger on Custom_Group__c (after insert, after update, before delete) {
    ...
    TriggerSFSyncAPI syncJob = new TriggerSFSyncAPI('Custom_Group__c', new List<String>(), '', new List<SObject>());

    // Handle "after insert" and "after update"
    if (Trigger.isInsert || Trigger.isUpdate) {
        for (Integer i = 0; i < Trigger.new.size(); i++) {
            Custom_Group__c newStudent = Trigger.new[i];
            Custom_Group__c oldStudent = Trigger.isUpdate ? Trigger.old[i] : null;
    ...
    if (Trigger.isDelete) {
        for (Custom_Group__c oldStudent : Trigger.old) {
