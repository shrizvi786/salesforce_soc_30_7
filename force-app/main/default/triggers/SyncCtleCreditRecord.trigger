trigger SyncCtleCreditRecord on CTLE_Credit__c (after insert, after update, before delete) {
...
TriggerSFSyncAPI.sendData('Contact',JSON.serialize(contact),'actionUpdate');  
    ...
    if (trigger.isInsert) {
        for (CTLE_Credit__c ctle : Trigger.new) {
        ...
        for (CTLE_Credit__c ctle : Trigger.old) {
