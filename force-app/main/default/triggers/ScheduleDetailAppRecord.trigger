trigger ScheduleDetailAppRecord on Schedule_Details__c (after insert, after update, before delete) {
    TriggerSFSyncAPI job = new TriggerSFSyncAPI('Schedule_Details__c', new List<String>(), '',new List<SObject>());
    if (trigger.isInsert) {
        for (Schedule_Details__c sch : Trigger.new) {
        ...
        for (Schedule_Details__c sch : Trigger.old) {
